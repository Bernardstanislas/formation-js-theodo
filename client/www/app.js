...
/***/ function(module, exports, __webpack_require__) {
	...
	var addItem = function addItem(items, newItem) {
	  return [].concat(_toConsumableArray(items), [_extends({ id: new Date().getTime() }, newItem)]);
	};
	var removeItem = function removeItem(items, itemId) {
	  return items.filter(function (_ref) {
	    var id = _ref.id;
	    return id !== itemId;
	  });
	};
	var modifyItem = function modifyItem(items, modifiedItem) {
	  return [].concat(_toConsumableArray(items.filter(function (_ref2) {
	    var id = _ref2.id;
	    return id !== modifiedItem.id;
	  })), [modifiedItem]);
	};

	angular.module('todo-list', ['lbServices']).controller('TodoListController', function ($scope, $http, Todo) {
	  $scope.title = 'Ma Todo liste';
	  $scope.todos = [];

	  var updateTodos = function updateTodos() {
	    Todo.find().$promise.then(function (todos) {
	      $scope.todos = todos;
	    });
	  };
	  ...
	  $scope.addItem = function () {
	    Todo.create({
	      ...
	      checked: false,
	      priority: 1
	    }).$promise.then(function (newTodo) {
	      $scope.todos = [].concat(_toConsumableArray($scope.todos), [newTodo]);
	    });
	    $scope.query = '';
	  };

	  $scope.removeItem = function (id) {
	    Todo.deleteById({ id: id }).$promise.then(updateTodos);
	  };

	  $scope.modifyItem = function (item) {
	    var modifiedTask = prompt('Modify todo task', item.task);
	    item.task = modifiedTask;
	    item.$save();
	  };
	});

	module.exports = { addItem: addItem, removeItem: removeItem, modifyItem: modifyItem };
...
/***/ function(module, exports) {
	...
	(function (window, angular, undefined) {
	   ...
	   *
	   */
	  module.factory("Todo", ['LoopBackResource', 'LoopBackAuth', '$injector', function (Resource, LoopBackAuth, $injector) {
	    var R = Resource(urlBase + "/todos/:id", { 'id': '@id' }, {
